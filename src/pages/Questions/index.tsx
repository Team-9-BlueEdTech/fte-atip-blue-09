...
const Question = ({
  ...
  return (
   ...
   {/* <Header/>*/}
      ...
      <S.InputBox type="checkbox" checked={editable} onChange={handleEditable} />
      <S.SpanQuest>
        <Button text='Remove Question'   onClick={removeQuestion} value={id}/>
      </S.SpanQuest>
      <ul>
        {options.map((o, i) => {
          if (i !== options.length - 1)
            return (
              <li key={o.id}>
                ...
                />
                <span>
                  <button onClick={removeOption} value={o.id}>
                    Remove Option
                  </button>
                </span>
              </li>
        ...
        <Button text='Create option' variant="add" onClick={addOption}/>
      </ul>
    </S.Container>
  );
};
...
const CensusQuestions = (): JSX.Element => {
  ...
  function getBaseQuestions(): void {
    api
      .get("/questions")
      .then((res) => {
        const baseQuestions: QuestionSubmit[] = res.data.map(
          (q: QuestionMap) => {
            return {
              id: q.id,
              text: q.question,
  ...
  function submit(e: React.MouseEvent<HTMLButtonElement, MouseEvent>): void {
    e.preventDefault();
    const mapped: QuestionMap[] = questions
      .map((q): QuestionMap | null => {
        if (q.wasEdited)
          return {
            id: q.id,
            question: q.text,
  ...
  return (
    <>
      ...
      <Button text='Add Question' variant="add" onClick={() => addQuestion()}/>
